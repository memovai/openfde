...
- **Un espacio de trabajo markdown-first al estilo Obsidian con cuatro pestañas.** `openfde serve` abre una interfaz local: **Note** (cada entidad, episodio y tarea como nota markdown — árbol jerárquico, [[wiki-enlaces]], citas en línea, más Vistas que reflejan las proyecciones de la CLI), **Ontology** (el grafo de entidades en un layout jerárquico determinista), **Todo** (un kanban sobre la máquina de estados de tareas — arrastra una tarjeta para transicionarla, movimientos ilegales rechazados) y **Canvas** (tarjetas markdown libres para el pensamiento previo a la estructura). Los humanos usan el workspace; los agents, la CLI.
...
| `openfde canvas show/add` | El lienzo de tarjetas libre del engagement (editado por arrastre en la pestaña Canvas del webui) |
