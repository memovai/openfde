...
- **Markdown ファーストの Obsidian スタイル・ワークスペース。** `openfde serve` で開くローカル UI では、すべてのエンティティ、エピソード、タスクが Markdown ノートです——階層ツリー、[[wiki リンク]]、インライン引用。さらに力学グラフと、CLI プロジェクションを鏡写しにしたビュー（インタビューガイド、データマップ、アセットライブラリ）。人間はワークスペース、agent は CLI。
